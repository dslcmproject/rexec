...
class TestServer(unittest.TestCase):
    ...
    def test_ok(self):
        result = rexec.check_call(ADDRESS, AUTHKEY, "action-ok", {"name": "first-request"})
        self.assertEqual(result, {"name": "first-response"})
        result = rexec.check_call(ADDRESS, AUTHKEY, "action-ok", {"name": "second-request"})
        self.assertEqual(result, {"name": "second-response"})

    def test_sendfile(self):
        result = rexec.check_call(ADDRESS, AUTHKEY, "action-file", {"name": "first-request"})
    ...
    def test_error(self):
        with self.assertRaises(rexec.RemoteException) as ex:
            rexec.check_call(ADDRESS, AUTHKEY, "action-error", {"name": "first-request"})
        self.assertEqual(str(ex.exception), "b'!!!ERROR!!!'")

    def test_exception(self):
        with self.assertRaises(rexec.RemoteException) as ex:
            rexec.check_call(ADDRESS, AUTHKEY, "action-exception", {"name": "first-request"})
        self.assertIn("AssertionError: Invalid argument", str(ex.exception))

    def test_timeout(self):
        with self.assertRaises(rexec.RemoteException) as ex:
            rexec.check_call(ADDRESS, AUTHKEY, "action-timeout", {"name": "first-request"})
        self.assertEqual("TIMEOUT", str(ex.exception))

    def test_returncode(self):
        with self.assertRaises(rexec.RemoteException) as ex:
            rexec.check_call(ADDRESS, AUTHKEY, "action-returncode", {"name": "first-request"})
        self.assertIn("INVALID RETURNCODE", str(ex.exception))

    def test_invalid_auth(self):
        self.assertRaises(
            rexec.AuthenticationError, rexec.check_call, ADDRESS, b"invalid password", "dummy", {"name": "dummy"})

    def test_invalid_action(self):
        with self.assertRaises(rexec.RemoteException) as ex:
            rexec.check_call(ADDRESS, AUTHKEY, "action-invalid", {"name": "first-request"})
